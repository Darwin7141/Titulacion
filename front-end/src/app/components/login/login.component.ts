...
export class LoginComponent implements OnInit {
  ...
  abrirRegistro() {
    ...
    this.dialog.open(RegistroClienteComponent, {
      width: '640px',
      autoFocus: false,
  ...
  onServicioChange(event: any, serv: any) {
    if (event.target.checked) {
      ...
      this.formData.serviciosSeleccionados =
        this.formData.serviciosSeleccionados.filter((id) => id !== serv.nombre);
    }
  }
  ...
  enviarFormulario() {
    ...
    this.contactoService.enviarContacto(this.formData).subscribe({
      next: (resp) => {
        ...
        alert('Tu solicitud ha sido enviada. Pronto nos comunicaremos contigo');
  ...
  async generarPDFServiciosMenus() {
    ...
    const drawPage = () => {
      ...
      doc.setTextColor('#A45B35');
      const titleSize = this.fitFontSizeToMaxLine(
        doc,
        titleLines,
        40,
        maxTitleW,
      ...
      const lastTitleY = this.drawCenteredLines(
        doc,
        titleLines,
        pageW / 2,
        TITLE_Y,
    ...
    const drawMenuRow = async (menu: any) => {
      ...
      try {
        if (menu.fotoMenuUrl) {
          const base64 = await this.getBase64ImageRounded(
            menu.fotoMenuUrl,
            IMG.w,
            IMG.h,
            IMG.r
      ...
      const nameLines = doc.splitTextToSize(
        String(menu.nombre || '').toUpperCase(),
        textMax
      );
      const nameY =
        centerY - ((nameLines.length - 1) * LINE_H) / 2 + FS_NAME * 0.35;
      doc.text(nameLines, textX, nameY);

      // precio
      const FS_PRICE = 12;
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(FS_PRICE);
      doc.setTextColor(PRICE_COLOR);
  ...
  solicitarRecuperacion() {
    ...
    this.recuperarService
      .solicitarRecuperacion({ correo: this.correoRecuperar })
